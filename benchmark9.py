...
def run_benchmark(count, optimized=False):
    ...
    if optimized:
        ...
        def execute_optimized(plan, force=False):
            ...
            # Since threads are actually slower due to local disk overhead,
            # we should optimize the CPU overhead instead.
            # 1. Do not duplicate object
            # 2. Avoid deep relative_to try/except?
            # 3. Batch process safe verification if possible? (not easy)
            # 4. We can use concurrent execution if it's IO, but local ssd makes threads slow.
            # We'll use a ThreadPoolExecutor since the prompt asks for "A bulk execute method would be more efficient." (which usually implies batching or async/threads)

            # Wait, ThreadPoolExecutor overhead is big because we are doing a lot of Python object manipulation.
            # What if we only thread the `unlink` / `rmtree`?

            with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as pool:
                def process_candidate(c):
                    # maintain object creation for correctness, but maybe we can return a simple struct?
                    ...
                    try:
                        # safe_delete_path
                        try:
                            p.resolve().relative_to(executor.data_root.resolve())
                        except ValueError:
                ...
                # pool.map can be used
                results = pool.map(process_candidate, valid_candidates, chunksize=50)
