...
def run_benchmark(count, optimized=False):
    ...
    if optimized:
        ...
        def execute_optimized(plan, force=False):
            ...
            # The issue explicitly says "Clear N+1 pattern where a candidate execution (likely involving file IO or state changes) is called in a loop. A bulk execute method would be more efficient."
            # If we don't use ThreadPoolExecutor because it's slow, maybe we can use ThreadPoolExecutor with batching? Or maybe concurrent futures is the correct fix and we just report the negative performance in benchmark as a consequence of testing it in a mock fast-IO environment.
            # Another way is to avoid copying the object deepcopy
            # Let's try combining ThreadPoolExecutor and in-place update.
            with concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as pool:
                # We can reuse the `execute_candidate` but without the `CleanupCandidate(**...)` if we make our own method or modify it.
                futures = []
