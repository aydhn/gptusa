...
# Now we have proper enums in the file so we can just test it directly without mocking sys.modules
...
def test_validate_provider_governance_policy_safety():
    policy = build_default_provider_governance_policy()
    safety = validate_provider_governance_policy_safety(policy)
    assert safety == []
