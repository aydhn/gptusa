...
def test_portfolio_risk_payload_has_forbidden_fields():
    ...
    assert (
        portfolio_risk_payload_has_forbidden_fields({"sandbox_optimizer_weight": 0.5})
        is False
    )


def test_pandas_import_error_fallback():
    import usa_signal_bot.portfolio.risk_reporting.portfolio_risk_safety_validator as prsv

    with patch.dict("sys.modules", {"pandas": None}):
        importlib.reload(prsv)
        assert prsv.pd is None
    # Reload again outside the patch to restore the module state and prevent test pollution
    importlib.reload(prsv)
