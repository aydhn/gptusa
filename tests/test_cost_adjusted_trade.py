...
def test_apply_costs_to_trade():
    ...
    sim = FillSimulationResult(
        fill_id=create_fill_simulation_id("SPY"),
        symbol="SPY",
        created_at_utc=datetime.now().isoformat(),
        side=TransactionSide.BUY,
        ...
        simulated_filled_quantity=10,
        simulated_filled_notional_usd=1005.0,
        status=FillSimulationStatus.FILLED,
        cost_breakdown=brk,
        market_impact=None,
        warnings=[],
        errors=[],
    ...
    inp = CostAdjustedTradeInput("SPY", TransactionSide.BUY, 100.0, 10.0, 1000.0, sim)
    res = apply_costs_to_trade(inp)
