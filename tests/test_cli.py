...
def test_provider_info_command():
    ...
    import subprocess
    result = subprocess.run([sys.executable, "-m", "usa_signal_bot", "provider-info"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_provider_list_command():
    import subprocess
    result = subprocess.run([sys.executable, "-m", "usa_signal_bot", "provider-list"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "Registered Providers" in result.stdout

def test_provider_check_command():
    import subprocess
    result = subprocess.run([sys.executable, "-m", "usa_signal_bot", "provider-check"], capture_output=True, text=True)
    assert result.returncode == 0
    assert "All guard checks passed" in result.stdout

def test_provider_plan_command():
    import subprocess
    result = subprocess.run([sys.executable, "-m", "usa_signal_bot", "provider-plan", "--symbols", "AAPL,MSFT", "--timeframe", "1d"], capture_output=True, text=True)
    assert result.returncode == 0
...
def test_provider_mock_fetch_command():
    import subprocess
    result = subprocess.run([sys.executable, "-m", "usa_signal_bot", "provider-mock-fetch", "--symbols", "AAPL", "--timeframe", "1d"], capture_output=True, text=True)
    assert result.returncode == 0
