import pandas as pd
from usa_signal_bot.ml_research.ensemble_evaluation.offline_ensemble_prediction_generator import generate_offline_ensemble_predictions
...
def test_generate_offline_ensemble_predictions():
    ...
    artifacts = generate_offline_ensemble_predictions([spec], df)

    assert len(artifacts) == 1
    assert artifacts[0].offline_evaluation_only is True
    assert artifacts[0].live_inference_output is False
