...
def test_write_session_validation_result_json_error(tmp_path):
    with patch("usa_signal_bot.calendar.calendar_store.open") as mock_open:
        ...
        with pytest.raises(
            DataStorageError, match="Failed to write session validation"
        ):
            write_session_validation_result_json(
                tmp_path / "calendar" / "session_validations" / "val_1.json", mock_res
            )
...
def test_read_calendar_review_result_json_success(tmp_path):
    import json
    from usa_signal_bot.calendar.calendar_store import read_calendar_review_result_json

    file_path = tmp_path / "review.json"
    data = {"validation_id": "val_1", "status": "ok"}

    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(data, f)

    result = read_calendar_review_result_json(file_path)
    assert result == data
...
def test_read_calendar_review_result_json_not_found(tmp_path):
    ...
    with pytest.raises(DataStorageError, match="Calendar review file not found"):
        read_calendar_review_result_json(file_path)
...
def test_read_calendar_review_result_json_error(tmp_path):
    ...
    with pytest.raises(DataStorageError, match="Failed to read calendar review"):
        read_calendar_review_result_json(file_path)
