...
from usa_signal_bot.transaction_costs.cost_validation import (
    validate_no_live_execution_language_in_cost,
    validate_no_sensitive_data_in_cost_payload,
)
...
def test_validate_slippage_curve_report_error():
    ...
    from usa_signal_bot.transaction_costs.cost_validation import (
        validate_slippage_curve_report,
    )
...
def test_validate_transaction_cost_breakdown_report_error_path():
    ...
    from usa_signal_bot.transaction_costs.cost_validation import (
        validate_transaction_cost_breakdown_report,
    )

    with patch(
        "usa_signal_bot.transaction_costs.cost_validation.validate_transaction_cost_breakdown"
    ) as mock_validate:
...
def test_validate_fill_simulation_report_error_path():
    from unittest.mock import patch, MagicMock
    from usa_signal_bot.transaction_costs.cost_validation import (
        validate_fill_simulation_report,
    )

    with patch(
        "usa_signal_bot.transaction_costs.cost_validation.validate_fill_simulation_result"
    ) as mock_validate:
        mock_validate.side_effect = ValueError("simulated validation error")
        ...
        report = validate_fill_simulation_report(dummy_item)
