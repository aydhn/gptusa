...
from usa_signal_bot.strategies.signal_contract import create_watch_signal, WatchSignalParams

def test_signal_store_operations(tmp_path):
    d = signal_store_dir(tmp_path)
    assert d.exists()

    sig = create_watch_signal(WatchSignalParams("test", "AAPL", "1d", "2023-01-01T00:00:00Z", "r1"))
