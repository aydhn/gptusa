...
from usa_signal_bot.paper_observer.observer_runtime_context import (
    build_mock_observer_runtime_context,
)
...
def test_create_observer_audit_entry():
    params = ObserverAuditEntryParams(
        entity_type="Entity", entity_id="123", action="ACTION", rationale="Rationale"
    )
    entry = create_observer_audit_entry(params)
    assert entry.entity_type == "Entity"
    assert entry.action == "ACTION"


def test_audit_entry_from_observer_enrollment():
    enrollment = build_observer_enrollment(
        "cand_1", "ticket_1", "APPROVED_FOR_NEXT_NON_EXECUTING_STAGE"
    )
