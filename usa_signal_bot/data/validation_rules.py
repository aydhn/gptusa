...
def validate_volume(
    bar: OHLCVBar, allow_zero_volume: bool = True
    ...
    if bar.volume < 0:
        ...
        )  # still emit warning but handled upstream maybe? Spec says "zero volume default warning"
    return results
...
def validate_symbol(
    bar: OHLCVBar, expected_symbols: Optional[List[str]] = None
) -> List[ValidationRuleResult]:
    results = []
    if not bar.symbol:
        results.append(
            ...
            if sym_bars[i].timestamp_utc < sym_bars[i - 1].timestamp_utc:
                results.append(
                    ValidationRuleResult(
                        ...
                        symbol=sym,
                        timestamp_utc=sym_bars[i].timestamp_utc,
                        field="timestamp_utc",
                    )
                )
...
def validate_missing_symbols(
    bars: List[OHLCVBar], expected_symbols: List[str]
) -> List[ValidationRuleResult]:
    results = []
...
def validate_empty_dataset(bars: List[OHLCVBar]) -> List[ValidationRuleResult]:
    if not bars:
        return [
            ValidationRuleResult(
                "empty_dataset", False, ValidationSeverity.ERROR, "Dataset is empty"
            )
        ]
    return []
