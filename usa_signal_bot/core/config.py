...
from usa_signal_bot.core.config_schema import (
    AppConfig,
    FinalClosureConfig,
    Phase160FinalPolicyConfig,
    Phase160DeliveryDefaultsConfig,
    Phase160NotificationsConfig,
    FinalClosureConfig,
    Phase160FinalPolicyConfig,
    Phase160DeliveryDefaultsConfig,
    Phase160NotificationsConfig,
    RegimeMonitoringConfig,
    ...
    Phase118FeatureTableConfig,
    Phase118NotificationsConfig,
    CoreRuntimeAcceptanceConfig,
    AdvancedFoundationFreezeConfig,
    DataProviderExpansionKickoffGateConfig,
    Phase105NotificationsConfig,
)
...
def _apply_standard_configs(config: AppConfig, merged_cfg_dict: dict) -> None:
    standard_keys = [
        "project",
        "runtime",
        "data",
        ...
        "task_conflicts",
        "queue_executor",
        "taskqueue_notifications",
        "incident_notifications",
    ]
    for key in standard_keys:
        if key in merged_cfg_dict:
            obj = getattr(config, key)
            for k, v in merged_cfg_dict[key].items():
                setattr(obj, k, v)


def _apply_dataclass_configs(config: AppConfig, merged_cfg_dict: dict) -> None:
    if "regime_final_closure" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import RegimeFinalClosureConfig

        config.regime_final_closure = RegimeFinalClosureConfig(
            **merged_cfg_dict["regime_final_closure"]
        )
    if "phase135_closure_policy" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import Phase135ClosurePolicyConfig

        config.phase135_closure_policy = Phase135ClosurePolicyConfig(
            **merged_cfg_dict["phase135_closure_policy"]
        )
    if "phase135_artifact_chain" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import Phase135ArtifactChainConfig

        config.phase135_artifact_chain = Phase135ArtifactChainConfig(
            **merged_cfg_dict["phase135_artifact_chain"]
        )
    if "phase135_freeze_seal" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import Phase135FreezeSealConfig

        config.phase135_freeze_seal = Phase135FreezeSealConfig(
            **merged_cfg_dict["phase135_freeze_seal"]
        )
    if "phase135_ml_kickoff" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import Phase135MLKickoffConfig

        config.phase135_ml_kickoff = Phase135MLKickoffConfig(
            **merged_cfg_dict["phase135_ml_kickoff"]
        )
    if "phase135_notifications" in merged_cfg_dict:
        from usa_signal_bot.core.config_schema import Phase135NotificationsConfig

        config.phase135_notifications = Phase135NotificationsConfig(
            **merged_cfg_dict["phase135_notifications"]
        )

    from usa_signal_bot.core.config_schema import (
        RuntimeLifecycleConfig,
        Phase104StartupChecksConfig,
        Phase104ReadinessGateConfig,
        Phase104NotificationsConfig,
        CoreRuntimeAcceptanceConfig,
        AdvancedFoundationFreezeConfig,
        DataProviderExpansionKickoffGateConfig,
        Phase105NotificationsConfig,
    ...
    if "runtime_lifecycle" in merged_cfg_dict:
        config.runtime_lifecycle = RuntimeLifecycleConfig(
            **merged_cfg_dict["runtime_lifecycle"]
        )
    if "phase104_startup_checks" in merged_cfg_dict:
        config.phase104_startup_checks = Phase104StartupChecksConfig(
            **merged_cfg_dict["phase104_startup_checks"]
        )
    if "phase104_readiness_gate" in merged_cfg_dict:
        config.phase104_readiness_gate = Phase104ReadinessGateConfig(
            **merged_cfg_dict["phase104_readiness_gate"]
        )
    if "phase104_notifications" in merged_cfg_dict:
        config.phase104_notifications = Phase104NotificationsConfig(
            **merged_cfg_dict["phase104_notifications"]
        )

    if "core_runtime_acceptance" in merged_cfg_dict:
        config.core_runtime_acceptance = CoreRuntimeAcceptanceConfig(
            **merged_cfg_dict["core_runtime_acceptance"]
        )
    if "advanced_foundation_freeze" in merged_cfg_dict:
        config.advanced_foundation_freeze = AdvancedFoundationFreezeConfig(
            **merged_cfg_dict["advanced_foundation_freeze"]
        )
    if "data_provider_expansion_kickoff_gate" in merged_cfg_dict:
        config.data_provider_expansion_kickoff_gate = (
            DataProviderExpansionKickoffGateConfig(
                **merged_cfg_dict["data_provider_expansion_kickoff_gate"]
            )
        )
    if "phase105_notifications" in merged_cfg_dict:
        config.phase105_notifications = Phase105NotificationsConfig(
    ...
    if "phase133_monitoring_policy" in merged_cfg_dict:
        config.phase133_monitoring_policy = Phase133MonitoringPolicyConfig(
            **merged_cfg_dict["phase133_monitoring_policy"]
        )
    if "phase133_drift_tracking" in merged_cfg_dict:
        config.phase133_drift_tracking = Phase133DriftTrackingConfig(
            **merged_cfg_dict["phase133_drift_tracking"]
        )
    if "phase133_degradation_diagnostics" in merged_cfg_dict:
        config.phase133_degradation_diagnostics = Phase133DegradationDiagnosticsConfig(
            **merged_cfg_dict["phase133_degradation_diagnostics"]
        )
    if "phase133_notifications" in merged_cfg_dict:
        config.phase133_notifications = Phase133NotificationsConfig(
            **merged_cfg_dict["phase133_notifications"]
        )


def _apply_advanced_features_configs(config: AppConfig, merged_cfg_dict: dict) -> None:
    if "advanced_features" in merged_cfg_dict:
        ...
        config.advanced_features = AdvancedFeaturesConfig(
            ...
            require_phase117_core_indicators=af_dict.get(
                "require_phase117_core_indicators", True
            ),
            advanced_volatility_enabled=af_dict.get(
                "advanced_volatility_enabled", True
            ),
            ...
            multi_symbol_feature_table_enabled=af_dict.get(
                "multi_symbol_feature_table_enabled", True
            ),
            write_advanced_feature_reports=af_dict.get(
                "write_advanced_feature_reports", True
            ),
            warn_not_investment_advice=af_dict.get("warn_not_investment_advice", True),
            warn_phase118_is_not_activation=af_dict.get(
                "warn_phase118_is_not_activation", True
            ),
            warn_advanced_features_are_not_trade_signals=af_dict.get(
                "warn_advanced_features_are_not_trade_signals", True
            ),
        )
    ...
    if "phase118_feature_table" in merged_cfg_dict:
        ...
        config.advanced_features.feature_table = Phase118FeatureTableConfig(
            ...
            overwrite_feature_tables_default=fd.get(
...
def load_app_config(config_dir: Optional[Path] = None) -> AppConfig:
    ...
    try:
        ...
        _apply_dataclass_configs(config, merged_cfg_dict)
        _apply_standard_configs(config, merged_cfg_dict)
        _apply_advanced_features_configs(config, merged_cfg_dict)
...
def load_paper_mode_dry_admission_gate_config(data: dict):
    ...
    return PaperModeDryAdmissionGateConfig(
        ...
        warn_shadow_replay_is_metadata_only=d.get(
            "warn_shadow_replay_is_metadata_only", True
        ),
        warn_board_evidence_freeze_is_metadata_only=d.get(
            "warn_board_evidence_freeze_is_metadata_only", True
        ),
        warn_dry_admission_gate_is_not_activation=d.get(
...
def load_shadow_launch_blocker_replay_config(data: dict):
    ...
    return ShadowLaunchBlockerReplayConfig(
        ...
        require_shadow_launch_blocker_events=d.get(
...
def load_final_paper_mode_dry_admission_gate_config(data: dict):
    ...
    return FinalPaperModeDryAdmissionGateConfig(
        ...
        require_dry_admission_assertions=d.get(
...
def load_dry_admission_gate_safety_config(data: dict):
    ...
    return DryAdmissionGateSafetyConfig(
        ...
        block_on_paper_state_mutation_risk=d.get(
            "block_on_paper_state_mutation_risk", True
        ),
        block_on_telegram_real_send_risk=d.get(
            "block_on_telegram_real_send_risk", True
        ),
        block_on_production_config_write_risk=d.get(
            "block_on_production_config_write_risk", True
        ),
        block_on_active_paper_enable_risk=d.get(
        ...
        block_on_activation_allowed_risk=d.get(
            "block_on_activation_allowed_risk", True
        ),
        block_on_transition_allowed_risk=d.get(
        ...
        block_on_board_evidence_freeze_failed=d.get(
            "block_on_board_evidence_freeze_failed", True
        ),
        block_on_dry_admission_assertion_failed=d.get(
