    ...
    return [
        {
            ...
            "applies_to_experiment_kinds": [
                BaselineExperimentKind.CLASSIFICATION_BASELINE,
                BaselineExperimentKind.REGIME_CONTEXT_BASELINE,
            ],
    ...
    return [
        {
            ...
            "applies_to_experiment_kinds": [
                BaselineExperimentKind.REGRESSION_BASELINE,
                BaselineExperimentKind.VOLATILITY_BASELINE,
            ],
...
def metric_spec_by_kind(
    kind: EvaluationMetricKind, specs: Optional[List[EvaluationMetricSpec]] = None
) -> Optional[EvaluationMetricSpec]:
...
def validate_evaluation_metric_specs(items: List[EvaluationMetricSpec]) -> List[str]:
    ...
    for item in items:
        ...
        if (
            item.produces_trade_signal
            or item.produces_order_decision
            or item.produces_portfolio_weights
        ):
            errors.append(
                f"Metric {item.metric_name} produces trading signals or orders"
            )
    return errors


def evaluation_metric_specs_summary(
    items: List[EvaluationMetricSpec],
) -> Dict[str, Any]:
...
def evaluation_metric_specs_to_text(
    items: List[EvaluationMetricSpec], limit: int = 300
) -> str:
