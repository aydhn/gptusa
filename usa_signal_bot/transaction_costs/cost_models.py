...
def transaction_cost_input_to_dict(payload: TransactionCostInput) -> dict:
    return {
        ...
        "side": payload.side.value if isinstance(payload.side, TransactionSide) else str(payload.side),
        ...
        "liquidity_status": payload.liquidity_status.value if hasattr(payload.liquidity_status, 'value') else payload.liquidity_status,
...
def transaction_cost_breakdown_to_dict(item: TransactionCostBreakdown) -> dict:
    return {
        ...
        "side": item.side.value if isinstance(item.side, TransactionSide) else str(item.side),
        ...
        "status": item.status.value if isinstance(item.status, CostAdjustmentStatus) else str(item.status),
        "realism_status": item.realism_status.value if isinstance(item.realism_status, CostRealismStatus) else str(item.realism_status),
...
def slippage_curve_to_dict(item: SlippageCurve) -> dict:
    return {
        ...
        "symbol": item.symbol,
        "curve_type": item.curve_type.value if isinstance(item.curve_type, SlippageCurveType) else str(item.curve_type),
        "created_at_utc": item.created_at_utc,
...
def market_impact_estimate_to_dict(item: MarketImpactEstimate) -> dict:
    return {
        ...
        "status": item.status.value if isinstance(item.status, MarketImpactStatus) else str(item.status),
        "order_size_class": item.order_size_class.value if isinstance(item.order_size_class, OrderSizeClass) else str(item.order_size_class),
        "warnings": item.warnings,
        "errors": item.errors,
        "metadata": item.metadata
    }
...
def fill_simulation_result_to_dict(item: FillSimulationResult) -> dict:
    return {
        ...
        "status": item.status.value if isinstance(item.status, FillSimulationStatus) else str(item.status),
        "cost_breakdown": transaction_cost_breakdown_to_dict(item.cost_breakdown) if item.cost_breakdown else None,
        "market_impact": market_impact_estimate_to_dict(item.market_impact) if item.market_impact else None,
        "warnings": item.warnings,
        "errors": item.errors,
        "metadata": item.metadata
    }
...
def cost_adjusted_trade_result_to_dict(item: CostAdjustedTradeResult) -> dict:
    return {
        ...
        "fill_result": fill_simulation_result_to_dict(item.fill_result) if item.fill_result else None,
        "status": item.status.value if isinstance(item.status, CostAdjustmentStatus) else str(item.status),
        "warnings": item.warnings,
        "errors": item.errors,
        "metadata": item.metadata
...
def transaction_cost_review_to_dict(item: TransactionCostReview) -> dict:
    return {
        ...
        "created_at_utc": item.created_at_utc,
        "report_type": item.report_type.value if isinstance(item.report_type, TransactionCostReportType) else str(item.report_type),
        "symbols": item.symbols,
        "cost_breakdowns": [transaction_cost_breakdown_to_dict(b) for b in item.cost_breakdowns],
        "impact_estimates": [market_impact_estimate_to_dict(i) for i in item.impact_estimates],
        "fill_results": [fill_simulation_result_to_dict(f) for f in item.fill_results],
        "adjusted_trade_results": [cost_adjusted_trade_result_to_dict(r) for r in item.adjusted_trade_results],
...
def validate_transaction_cost_input(payload: TransactionCostInput) -> None:
    ...
    if payload.participation_rate_pct is not None and payload.participation_rate_pct < 0:
...
def validate_fee_schedule_proxy(item: FeeScheduleProxy) -> None:
    ...
    if item.max_commission_pct_notional is not None and item.max_commission_pct_notional < 0:
...
def validate_cost_adjusted_trade_result(item: CostAdjustedTradeResult) -> None:
    ...
    if item.gross_pnl_usd is not None and item.total_cost_usd is not None and item.net_pnl_usd is not None:
        expected_net = item.gross_pnl_usd - item.total_cost_usd
        if abs(expected_net - item.net_pnl_usd) > 0.001:
            raise ValueError(f"net_pnl_usd {item.net_pnl_usd} does not match gross {item.gross_pnl_usd} - cost {item.total_cost_usd}")
