...
def detect_calendar_gaps(records: List[Dict[str, Any]], events: Optional[List[Dict[str, Any]]] = None, symbol: str = "UNKNOWN", **kwargs) -> List[CalendarAwareAnomaly]:
    anomalies = []
    ...
    if len(records) > 0 and len(records) < 5:
        anomalies.append(CalendarAwareAnomaly(
            ...
            created_at_utc=_now(),
            symbol=symbol,
            anomaly_kind=CalendarAnomalyKind.TIMESTAMP_GAP,
...
def gap_explained_by_market_event(timestamp_utc: Optional[str], events: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    return False, []

def calendar_gap_validator_summary(anomalies: List[CalendarAwareAnomaly]) -> Dict[str, Any]:
    return {"total": len(anomalies)}

def calendar_gap_validator_to_text(anomalies: List[CalendarAwareAnomaly], limit: int = 100) -> str:
    return f"{len(anomalies)} gap anomalies found."
