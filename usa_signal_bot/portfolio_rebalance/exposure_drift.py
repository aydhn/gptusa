...
def drift_for_exposure(
    ...
    if total_equity_usd and total_equity_usd > 0 and current_value is not None and target_value is not None:
        curr_pct = (current_value / total_equity_usd) * 100.0
        tgt_pct = (target_value / total_equity_usd) * 100.0
        pct_drift = tgt_pct - curr_pct
        abs_drift = abs(pct_drift)
        severity = classify_drift_severity(abs_drift, low=threshold/3, moderate=threshold, high=threshold*2, critical=threshold*3)
    else:
        warnings.append(f"Insufficient data to calculate drift percentage for {name}")

    return DriftMeasurement(
        drift_id=create_drift_measurement_id(name),
...
def calculate_long_short_exposure_drift(current: CurrentPortfolioState, target: TargetPortfolioState, threshold: float = 3.0) -> List[DriftMeasurement]:
    ...
    return [
        drift_for_exposure("Long Exposure", current_long, target_long, equity, DriftType.LONG_EXPOSURE, threshold),
        drift_for_exposure("Short Exposure", current_short, target_short, equity, DriftType.SHORT_EXPOSURE, threshold)
    ]
