...
from usa_signal_bot.performance.baseline_collectors import (
    normalize_scheduler_result_to_sample,
)
...
def performance_sample_from_scheduler_run(
    payload: Dict[str, Any],
) -> CurrentPerformanceSample:
    return normalize_scheduler_result_to_sample(payload)


def scheduler_hints_from_performance_gate(
    result: PerformanceAcceptanceGateResult,
) -> Dict[str, Any]:
    hints = {"action": "PROCEED", "delay_minutes": 0, "reason": None}

    if result.status == BaselineComparisonStatus.BLOCKED:
        hints["action"] = "BLOCK"
        hints["delay_minutes"] = 120
        hints["reason"] = (
            "Performance BLOCKED. Temporarily suspending scheduler dispatch."
        )
    elif result.status == BaselineComparisonStatus.FAIL:
        hints["action"] = "DELAY"
        hints["delay_minutes"] = 60
        hints["reason"] = (
            "Performance FAIL. Delaying scheduler tasks to clear local resources."
        )
...
def annotate_scheduler_plan_with_performance_gate(
    plan: Any, gate_result: PerformanceAcceptanceGateResult
) -> Any:
    try:
        ...
        logger.error(
            "Error annotating scheduler plan with performance gate", exc_info=e
        )
    return plan
