...
def extract_engine_readiness_certificate(
...
def _check_readiness(
    ...
    feature_factor_engine_final_closed = (
        ctx.get("feature_factor_engine_final_closed", False) if ctx else False
    )
...
def _check_activation_parameters(ctx: dict, errors: list[str]) -> bool:
    valid = True
    activation_allowed = ctx.get("activation_allowed", True) if ctx else True
    strategy_activation_allowed = (
        ctx.get("strategy_activation_allowed", True) if ctx else True
    )
    ...
    broker_execution_enabled = (
        ctx.get("broker_execution_enabled", True) if ctx else True
    )
    order_creation_enabled = ctx.get("order_creation_enabled", True) if ctx else True
    paper_state_mutation_enabled = (
        ctx.get("paper_state_mutation_enabled", True) if ctx else True
    )
    telegram_real_send_enabled = (
        ctx.get("telegram_real_send_enabled", True) if ctx else True
    )
    ...
    produces_portfolio_weights = (
        ctx.get("produces_portfolio_weights", True) if ctx else True
    )
    investment_advice = ctx.get("investment_advice", True) if ctx else True

    if (
        activation_allowed
        or strategy_activation_allowed
        or deployment_allowed
        or active_paper_enabled
        or broker_execution_enabled
        or order_creation_enabled
        or paper_state_mutation_enabled
        or telegram_real_send_enabled
        or scraping_enabled
        or html_parse_enabled
        or paid_api_enabled
        or dashboard_enabled
        or network_default_enabled
        or produces_trade_signal
        or produces_order_decision
        or produces_portfolio_weights
        or investment_advice
    ):
        errors.append(
            "Execution, Activation, Dashboard or Network functions are illegally enabled."
        )
        valid = False
...
def ingest_final_closure_review_payload(
    payload: dict[str, Any],
) -> FinalClosureIngestionResult:
    warnings = []
    errors = []
    ...
    valid = (
        _check_readiness(
            ctx,
            engine_certificate_ready,
            phase126_kickoff_gate_ready,
            ready_for_phase126,
            errors,
        )
        and valid
    )
    valid = _check_activation_parameters(ctx, errors) and valid
    ...
    result_kwargs = {
        ...
        "final_artifacts_ready": (
            ctx.get("final_artifacts_ready", False) if ctx else False
        ),
        "final_checks_passed": ctx.get("final_checks_passed", False) if ctx else False,
        "freeze_seal_ready": ctx.get("freeze_seal_ready", False) if ctx else False,
    ...
    return FinalClosureIngestionResult(**result_kwargs)


def ingest_latest_final_closure_review_from_store(
    data_root: Path,
) -> FinalClosureIngestionResult:
    from usa_signal_bot.feature_engine.final_closure.final_closure_store import (
        get_latest_final_closure_review,
    )
