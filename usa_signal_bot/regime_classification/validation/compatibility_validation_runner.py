...
from usa_signal_bot.core.enums import CompatibilityValidationRuleKind, RegimeContextValidationQuality, RegimeContextValidationRiskFlag
...
from usa_signal_bot.regime_classification.validation.compatibility_validation_specs import build_validation_rule

def validate_score_ranges(compatibility_results: list[dict[str, Any]], overlay_results: list[dict[str, Any]]) -> list[str]:
    errors = []
...
def count_context_categories(compatibility_results: list[dict[str, Any]]) -> dict[str, int]:
...
def validate_explained_contexts(compatibility_results: list[dict[str, Any]], diagnostics_profiles: list[dict[str, Any]]) -> dict[str, int]:
...
def build_compatibility_validation_rules(
    ...
    # 1. Alignment Review Valid
    rules.append(build_validation_rule(
    ...
    # 4. Classification valid (assuming true for mock)
    rules.append(build_validation_rule(
    ...
    rules.append(build_validation_rule(
        ...
        passed=not (ingestion.activation_allowed or ingestion.active_paper_enabled or ingestion.broker_execution_enabled),
        rationale="Must block execution outputs"
    ))
    rules.append(build_validation_rule(
        CompatibilityValidationRuleKind.NO_MODEL_TRAINING,
        passed=not (ingestion.model_training_used or ingestion.model_prediction_used),
...
def run_compatibility_validation(
    ...
    rules = build_compatibility_validation_rules(ingestion, compatibility_results, overlay_results, diagnostics_profiles)
    ...
    return CompatibilityValidationResult(
        ...
        quality=RegimeContextValidationQuality.HIGH if validation_passed else RegimeContextValidationQuality.INVALID,
...
def compatibility_validation_summary(result: CompatibilityValidationResult) -> dict[str, Any]:
    return {
        "passed": result.validation_passed,
        "rules_passed": f"{result.passed_rules}/{result.total_rules}"
    }

def compatibility_validation_to_text(result: CompatibilityValidationResult, limit: int = 300) -> str:
