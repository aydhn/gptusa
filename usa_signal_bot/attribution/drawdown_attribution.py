...
def calculate_running_equity(events: List[AttributionTradeEvent], starting_equity: float = 100000.0) -> List[Dict[str, Any]]:
    # Sort by timestamp if available, else retain input order
    sorted_events = sorted(events, key=lambda x: x.timestamp_utc or "")

    equity = starting_equity
    points = [{"timestamp": None, "equity": equity, "peak": equity, "drawdown": 0.0, "event_id": None}]
...
def calculate_event_drawdowns(equity_points: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return [p for p in equity_points if p["event_id"] is not None]

def drawdown_contribution_by_dimension(events: List[AttributionTradeEvent], dimension: AttributionDimension) -> List[RiskAttributionContribution]:
    ...
    for name, dd_sum in groups.items():
        contribs.append(RiskAttributionContribution(
            ...
            contribution_direction=ContributionDirection.NEGATIVE if dd_sum > 0 else ContributionDirection.NEUTRAL
        ))
    return sorted(contribs, key=lambda x: x.drawdown_contribution_usd or 0.0, reverse=True)

def identify_drawdown_contributors(events: List[AttributionTradeEvent], top_n: int = 10) -> List[RiskAttributionContribution]:
    return drawdown_contribution_by_dimension(events, AttributionDimension.SYMBOL)[:top_n]

def drawdown_attribution_to_text(contributions: List[RiskAttributionContribution], limit: int = 50) -> str:
    lines = [f"--- Drawdown Attribution ---"]
    for c in contributions[:limit]:
        lines.append(f"{c.name}: Drawdown Contribution: ${c.drawdown_contribution_usd or 0.0:.2f}")
    return "\n".join(lines)
