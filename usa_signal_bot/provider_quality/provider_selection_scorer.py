...
from usa_signal_bot.provider_quality.phase109_models import (
    ...
    SourceTrustProfile,
    create_provider_selection_score_id,
)


@dataclass
class ProviderSelectionParams:
    provider_name: str
    symbol: Optional[str]
    capability: str
...
def final_provider_selection_score(
    quality_score: float,
    trust_score: float,
    ...
    return (
        ...
        + (freshness_score * w_f)
        + (safety_score * w_s)
        + (availability_score * w_a)
    )


def provider_selection_status_from_score(
...
def provider_ranking_decision_from_score(
    score: float, blocked: bool = False
) -> ProviderRankingDecision:
    if blocked:
...
def build_provider_selection_score(
    ...
    if status in [
        ProviderSelectionScoreStatus.NOT_RECOMMENDED_FOR_DATA_USE,
        ProviderSelectionScoreStatus.BLOCKED,
    ]:
    ...
    return ProviderSelectionScore(
        ...
        created_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat() + "Z",
        provider_name=params.provider_name,
        symbol=params.symbol,
        capability=params.capability,
        data_quality_score_id=(
            params.quality_score.score_id if params.quality_score else None
        ),
        trust_profile_id=(
            params.trust_profile.profile_id if params.trust_profile else None
        ),
        quality_score=q_val,
        trust_score=t_val,
