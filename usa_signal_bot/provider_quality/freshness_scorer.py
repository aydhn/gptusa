...
def freshness_score_from_age(age_seconds: Optional[int], ttl_seconds: Optional[int]) -> float:
...
def score_freshness(fresh: bool, stale: bool, expired: bool = False, age_seconds: Optional[int] = None, ttl_seconds: Optional[int] = None, provider_name: str = "UNKNOWN", symbol: Optional[str] = None) -> DataQualityScoreComponent:
    risk_flags = []
    warnings = []

    if expired:
        score = 0.0
        warnings.append("Data is expired")
        risk_flags.append(ProviderQualityRiskFlag.CACHE_RECORD_STALE)
    elif stale:
        score = 40.0
        ...
        if score < 50:
            warnings.append("Data is relatively old")
