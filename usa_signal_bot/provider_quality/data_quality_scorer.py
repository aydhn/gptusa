...
from usa_signal_bot.provider_quality.scoring_policy import build_default_provider_quality_scoring_policy, normalize_scoring_weights, scoring_policy_component_weight
...
from usa_signal_bot.provider_quality.source_disagreement_scorer import score_source_agreement
from usa_signal_bot.provider_quality.outlier_penalty_scorer import score_outlier_profile
from usa_signal_bot.provider_quality.cache_reliability_scorer import score_cache_reliability
from usa_signal_bot.provider_quality.provider_safety_compliance_scorer import score_provider_safety_compliance


def data_quality_grade_from_score(score: float, blocked: bool = False) -> DataQualityGrade:
    if blocked:
...
def build_provider_data_quality_score(
    ...
    c_payload = cache_payload or {"status": "UNKNOWN", "checksum_present": False, "schema_valid": len(schema_errors)==0}
    saf_payload = safety_payload or {}

    c_comp = score_completeness(records, provider_name=provider_name, symbol=symbol)
    f_comp = score_freshness(f_payload.get("fresh", False), f_payload.get("stale", False),
                             f_payload.get("expired", False), f_payload.get("age_seconds"), f_payload.get("ttl_seconds"),
                             provider_name=provider_name, symbol=symbol)
    sv_comp = score_schema_validity(schema_errors, provider_name=provider_name, symbol=symbol)
    cont_comp = score_continuity(records, expected_interval="1d", provider_name=provider_name, symbol=symbol)
    sa_comp = score_source_agreement(sc_payload.get("disagreement_score"), sc_payload.get("status"), provider_name=provider_name, symbol=symbol)
    out_comp = score_outlier_profile(records, provider_name=provider_name, symbol=symbol)
    cr_comp = score_cache_reliability(c_payload.get("status"), c_payload.get("checksum_present", False), c_payload.get("schema_valid", True), provider_name=provider_name, symbol=symbol)
    ...
    components = [c_comp, f_comp, sv_comp, cont_comp, sa_comp, out_comp, cr_comp, sf_comp]

    return aggregate_quality_components(provider_name, symbol, capability, components, policy)
...
def provider_data_quality_score_to_text(score: ProviderDataQualityScore, limit: int = 100) -> str:
    lines = [
