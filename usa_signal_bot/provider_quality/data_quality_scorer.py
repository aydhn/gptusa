...
from usa_signal_bot.provider_quality.phase109_models import (
    ProviderDataQualityScorePayload,
...
def build_provider_data_quality_score(
    payload: ProviderDataQualityScorePayload,
) -> ProviderDataQualityScore:

    records = payload.records or []
    schema_errors = payload.schema_errors or []
    f_payload = payload.freshness_payload or {"fresh": False, "stale": False}
    sc_payload = payload.source_comparison_payload or {"disagreement_score": None}
    c_payload = payload.cache_payload or {
        "status": "UNKNOWN",
        "checksum_present": False,
        "schema_valid": len(schema_errors) == 0,
    }
    saf_payload = payload.safety_payload or {}

    c_comp = score_completeness(
        records, provider_name=payload.provider_name, symbol=payload.symbol
    )
    ...
    sv_comp = score_schema_validity(
        schema_errors, provider_name=payload.provider_name, symbol=payload.symbol
    )
