...
def create_observer_audit_entry(
    entity_type: str,
    entity_id: str,
    action: str,
    rationale: str,
...
def audit_entry_from_observer_enrollment(enrollment: PaperObserverEnrollment) -> ObserverAuditEntry:
    return create_observer_audit_entry(
        ...
        evidence_refs=[enrollment.planning_ticket_id] if enrollment.planning_ticket_id else [],
        safety_flags=enrollment.safety_flags
    )

def audit_entry_from_observer_session(session: ObserverRuntimeSession) -> ObserverAuditEntry:
    return create_observer_audit_entry(
...
def append_observer_audit_entry(entries: List[ObserverAuditEntry], entry: ObserverAuditEntry) -> List[ObserverAuditEntry]:
