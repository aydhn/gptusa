...
def create_trade_id(
    symbol: str, entry_time_utc: Optional[str], signal_id: Optional[str] = None
) -> str:
...
def calculate_trade_pnl(
    entry_fill: BacktestFill, exit_fill: BacktestFill
) -> Tuple[float, float]:
...
def calculate_trade_return_pct(
    entry_price: float, exit_price: float, direction: TradeDirection
) -> Optional[float]:
    if entry_price <= 0:
        return None
...
def _create_closed_trade(
    ...
    qty: float,
    signals_by_id: Optional[Dict[str, StrategySignal]] = None,
) -> BacktestTrade:
    gross_pnl, net_pnl = calculate_trade_pnl(entry_fill, exit_fill)
    return_pct = calculate_trade_return_pct(
        entry_fill.fill_price, exit_fill.fill_price, TradeDirection.LONG
    )
    ...
    entry_slippage_cost = (
        entry_fill.slippage_breakdown.get("total_slippage_cost", 0.0)
        if hasattr(entry_fill, "slippage_breakdown") and entry_fill.slippage_breakdown
        else 0.0
    )
    exit_slippage_cost = (
        exit_fill.slippage_breakdown.get("total_slippage_cost", 0.0)
        if hasattr(exit_fill, "slippage_breakdown") and exit_fill.slippage_breakdown
        else 0.0
    )
    ...
    return BacktestTrade(
        trade_id=create_trade_id(
            symbol, entry_fill.timestamp_utc, entry_fill.signal_id
        ),
        symbol=symbol,
...
def pair_long_fills_into_trades(
    fills: List[BacktestFill], signals_by_id: Optional[Dict[str, StrategySignal]] = None
) -> Tuple[List[BacktestTrade], List[BacktestTrade], List[str]]:
    ...
    for symbol, sym_fills in fills_by_symbol.items():
        ...
        for fill in sym_fills:
            if fill.side == BacktestOrderSide.BUY:
                ...
                if not open_buys:
                    warnings.append(
                        f"Oversell or missing BUY fill for {symbol} at {fill.timestamp_utc}"
                    )
                    continue
                ...
                if fill.quantity != entry_fill.quantity:
                    warnings.append(
                        f"Partial fill pairing not fully supported. Entry: {entry_fill.quantity}, Exit: {fill.quantity}"
                    )

                trade = _create_closed_trade(
                    symbol, entry_fill, fill, qty, signals_by_id
                )
...
def build_trade_ledger_from_fills(
    fills: List[BacktestFill], signals_by_id: Optional[Dict[str, StrategySignal]] = None
) -> TradeLedger:
    closed_trades, open_trades, warnings = pair_long_fills_into_trades(
        fills, signals_by_id
    )
...
def ledger_to_text(ledger: TradeLedger, limit: int = 20) -> str:
    ...
    for trade in sorted(
        ledger.closed_trades, key=lambda x: x.exit_time_utc or "", reverse=True
    )[:limit]:
        lines.append(
            f"  {trade.symbol} | {trade.direction.value} | Qty: {trade.quantity:.2f} | Net PnL: {trade.net_pnl:.2f} | Entry: {trade.entry_price:.2f} | Exit: {trade.exit_price:.2f}"
        )
...
def validate_trade(trade: BacktestTrade) -> None:
    ...
    if trade.status == TradeStatus.CLOSED and (
        not trade.exit_time_utc or trade.exit_price is None
    ):
