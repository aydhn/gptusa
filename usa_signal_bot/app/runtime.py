...
from usa_signal_bot.core.logging_config import (
    setup_logging,
    get_logger,
    log_system_event,
)
...
from usa_signal_bot.core.health import (
    run_health_checks,
    assert_health_ok,
...
def _setup_directories() -> None:
    """Ensure required directories and storage areas exist."""
    ensure_directories()
    try:
        ...
        logging.getLogger(__name__).warning("Storage module not fully loaded during directory setup: %s", e)


def _setup_logging(config: Any) -> tuple[logging.Logger, Path]:
    """Configure logging and return the logger and log directory path."""
    ...
    return logger, log_path_dir


def _create_runtime_context(
    config: Any, config_dir: Optional[Path], log_path_dir: Path
) -> RuntimeContext:
    ...
    return RuntimeContext(
        ...
        log_file_path=log_path_dir / config.logging.log_file,
        audit_log_path=get_audit_log_path(log_path_dir),
    )


def _verify_safe_mode(context: RuntimeContext, log_path_dir: Path) -> None:
    """Verify safe mode constraints, auditing any failures."""
    try:
...
def _log_initialization_events(
    config: Any, context: RuntimeContext, logger: logging.Logger, log_path_dir: Path
) -> None:
    """Create and log standard initialization events."""
    events_to_log = [
        create_event(
            "RUNTIME_STARTED",
            "INFO",
        ...
        create_event(
            "CONFIG_VALIDATED", "INFO", "Configuration loaded and validated.", "runtime"
        ),
        create_event(
        ...
        create_event(
            ...
            "security",
            context.as_summary_dict().get("safe_mode", {}),
        ),
...
def _run_initial_health_checks(
    context: RuntimeContext, logger: logging.Logger, log_path_dir: Path
) -> None:
    """Run startup health checks and update the context status."""
    health_results = run_health_checks(context)
    try:
        assert_health_ok(health_results)
        context.health_status = "HEALTHY"
        health_event = create_event(
            "HEALTH_CHECK_PASSED", "INFO", "Initial health check passed.", "runtime"
        )
    except Exception as e:
        context.health_status = "UNHEALTHY"
        health_event = create_event(
            "HEALTH_CHECK_FAILED", "ERROR", f"Health check failed: {e}", "runtime"
        )
...
def initialize_runtime(config_dir: Optional[Path] = None) -> RuntimeContext:
    ...
    # 5. Set up logging
    logger, log_path_dir = _setup_logging(config)

    # 6. Create RuntimeContext
    context = _create_runtime_context(config, config_dir, log_path_dir)

    # 7. Safe mode control
    _verify_safe_mode(context, log_path_dir)

    # 8. Create and log initialization events
    _log_initialization_events(config, context, logger, log_path_dir)

    # 9. Health check
    _run_initial_health_checks(context, logger, log_path_dir)
