...
def build_turnover_sandbox_diagnostics(
    ...
    if not previous_table:
        diags.append(
            PortfolioSandboxDiagnosticRecord(
                ...
                value=0.0,
                diagnostic_notes=[
                    "No previous table for turnover comparison. Defaulting to 0.0"
                ],
    ...
    curr_method_map = defaultdict(dict)
    for r in current_table.records:
        curr_method_map[r.method_kind][r.symbol] = r.normalized_sandbox_weight or 0.0

    prev_method_map = defaultdict(dict)
    for r in previous_table.records:
        prev_method_map[r.method_kind][r.symbol] = r.normalized_sandbox_weight or 0.0

    for method in methods:
        curr_map = curr_method_map[method]
        prev_map = prev_method_map[method]

        symbols = set(curr_map.keys()) | set(prev_map.keys())
        turnover = (
            sum([abs(curr_map.get(s, 0.0) - prev_map.get(s, 0.0)) for s in symbols])
            / 2.0
        )
...
def validate_turnover_sandbox_diagnostics(
    items: List[PortfolioSandboxDiagnosticRecord],
) -> List[str]:
    from usa_signal_bot.portfolio.construction.concentration_diagnostics import (
        validate_concentration_diagnostics,
    )

    return validate_concentration_diagnostics(items)


def turnover_sandbox_diagnostics_summary(
    items: List[PortfolioSandboxDiagnosticRecord],
) -> Dict[str, Any]:
    return {
        "count": len(items),
