...
from usa_signal_bot.portfolio.foundation.phase153_models import (
    PortfolioFoundationSafetyBoundaryResult, PortfolioFoundationSafetyBoundaryRule,
    PortfolioFoundationSafetyRuleKind
)

def build_portfolio_foundation_safety_boundary_rules(context_payload: dict[str, Any] | None = None) -> list[PortfolioFoundationSafetyBoundaryRule]:
    rules = []
    ...
    for kind in kinds:
        ...
        if context_payload:
            # Simple check for any unsafe flag
            for unsafe in ["live_trading_enabled", "paper_trading_enabled", "broker_execution_enabled", "real_order_creation_enabled", "paper_state_mutation_enabled", "telegram_real_send_enabled", "strategy_activation_allowed", "actual_portfolio_construction_executed", "actual_position_sizing_executed", "portfolio_optimization_enabled", "rebalancing_enabled", "target_weights_produced", "allocation_output_produced", "capital_deployment_allowed", "deployment_allowed", "network_used", "dashboard_started", "daemon_started", "scheduler_enabled"]:
                if context_payload.get(unsafe, False):
                    r.passed = False
...
def build_portfolio_foundation_safety_boundary_result(rules: list[PortfolioFoundationSafetyBoundaryRule]) -> PortfolioFoundationSafetyBoundaryResult:
    res = PortfolioFoundationSafetyBoundaryResult()
    res.rules = rules
    res.boundary_passed = all(r.passed for r in rules)
    return res

def validate_portfolio_foundation_safety_boundary_result(result: PortfolioFoundationSafetyBoundaryResult) -> list[str]:
    errors = []
    if not result.boundary_passed:
        errors.append("Safety boundary did not pass")
    for field in ["read_only_handoff_ingestion", "contract_only_portfolio_foundation", "no_actual_portfolio_construction", "no_position_sizing", "no_target_weights", "no_allocation_output", "no_capital_deployment", "no_portfolio_optimization", "no_rebalancing", "no_live_trading", "no_paper_trading", "no_broker_execution", "no_real_order_creation", "no_paper_state_mutation", "no_telegram_real_send", "no_strategy_activation", "no_deployment", "no_network", "no_dashboard", "no_daemon", "no_scheduler", "research_data_only"]:
        if not getattr(result, field):
            errors.append(f"Safety constraint {field} must be True")
    return errors

def portfolio_foundation_safety_boundary_passed(result: PortfolioFoundationSafetyBoundaryResult) -> bool:
    return result.boundary_passed

def portfolio_foundation_safety_boundary_summary(result: PortfolioFoundationSafetyBoundaryResult) -> dict[str, Any]:
    return {
        "passed": result.boundary_passed,
        "rule_count": len(result.rules)
    }

def portfolio_foundation_safety_boundary_to_text(result: PortfolioFoundationSafetyBoundaryResult, limit: int = 300) -> str:
