...
ALLOWED_CANDIDATE_COLUMNS = {
    ...
    "liquidity_score",
    "cost_score",
    "previous_sandbox_weight",
}


def build_optimizer_input_references(
    payloads: Dict[str, Any], dataframes: Optional[Dict[str, pd.DataFrame]] = None
) -> List[OptimizerInputReference]:
    refs = []
    if dataframes:
        for k, df in dataframes.items():
            ref = OptimizerInputReference(
                input_kind=OptimizerInputKind.OPTIMIZER_CANDIDATE_INPUTS,
                source_artifact_name=k,
            )
            ref.columns = list(df.columns)
            ref.row_count = len(df)
            ref.forbidden_columns_detected = detect_forbidden_optimizer_columns(
                ref.columns
            )
            if ref.forbidden_columns_detected:
                ref.errors.append(
                    f"Forbidden columns found: {ref.forbidden_columns_detected}"
                )
            else:
                ref.available = True
            refs.append(ref)
    ...
    for k, p in payloads.items():
        ref = OptimizerInputReference(
            input_kind=OptimizerInputKind.UNKNOWN, source_artifact_name=k
        )
...
def detect_forbidden_optimizer_columns(columns: List[str]) -> List[str]:
    return list(FORBIDDEN_COLUMNS.intersection(columns))
...
def optimizer_input_resolver_summary(
    items: List[OptimizerInputReference],
) -> Dict[str, Any]:
    return {"count": len(items), "available": sum(1 for i in items if i.available)}


def optimizer_input_resolver_to_text(
    items: List[OptimizerInputReference], limit: int = 300
) -> str:
