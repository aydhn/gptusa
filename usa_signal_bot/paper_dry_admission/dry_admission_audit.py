...
from usa_signal_bot.paper_dry_admission.dry_admission_safety_validator import (
    collect_dry_admission_safety_flags,
)


@dataclass
class DryAdmissionAuditEntryParams:
    entity_type: str
    entity_id: str
    action: str
    rationale: str
...
def create_dry_admission_audit_entry(
    params: DryAdmissionAuditEntryParams,
) -> DryAdmissionAuditEntry:
...
def audit_entry_from_dry_admission_run(
...
def audit_entry_from_write_lock_refresh(
    ...
    return create_dry_admission_audit_entry(
        params=DryAdmissionAuditEntryParams(
            ...
            evidence_refs=(
                [refresh.source_write_block_proof_id]
                if refresh.source_write_block_proof_id
                else []
            ),
...
def audit_entry_from_human_ledger(
...
def append_dry_admission_audit_entry(
    entries: List[DryAdmissionAuditEntry], entry: DryAdmissionAuditEntry
) -> List[DryAdmissionAuditEntry]:
    return entries + [entry]


def dry_admission_audit_summary(
    entries: List[DryAdmissionAuditEntry],
) -> dict[str, Any]:
    return {
...
def dry_admission_audit_to_text(
    entries: List[DryAdmissionAuditEntry], limit: int = 100
) -> str:
    lines = [f"Audit Entries: {len(entries)}"]
    for i, e in enumerate(entries[:limit]):
        lines.append(
            f"{i+1}. {e.action} on {e.entity_type} ({e.entity_id}) -> {e.decision or 'NO_DECISION'}"
        )
