...
@dataclass
class DryAdmissionGateAuditEntryParams:
    entity_type: str
    entity_id: str
    action: str
    rationale: str
...
def create_dry_admission_audit_entry(
    params: DryAdmissionGateAuditEntryParams,
) -> DryAdmissionGateAuditEntry:
...
def audit_entry_from_final_dry_admission_gate(
...
def audit_entry_from_shadow_replay_result(
    result: ShadowLaunchReplayResult,
) -> DryAdmissionGateAuditEntry:
    return create_dry_admission_audit_entry(
        params=DryAdmissionGateAuditEntryParams(
...
def audit_entry_from_board_evidence_freeze(
    bundle: BoardEvidenceFreezeBundle,
) -> DryAdmissionGateAuditEntry:
    return create_dry_admission_audit_entry(
        params=DryAdmissionGateAuditEntryParams(
...
def append_dry_admission_audit_entry(
    entries: List[DryAdmissionGateAuditEntry], entry: DryAdmissionGateAuditEntry
) -> List[DryAdmissionGateAuditEntry]:
    # Local only, no external
    return entries + [entry]


def dry_admission_audit_summary(
    entries: List[DryAdmissionGateAuditEntry],
) -> dict[str, Any]:
    return {"count": len(entries)}


def dry_admission_audit_to_text(
    entries: List[DryAdmissionGateAuditEntry], limit: int = 100
) -> str:
