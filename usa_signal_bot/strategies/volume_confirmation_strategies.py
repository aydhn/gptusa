...
from usa_signal_bot.strategies.rule_feature_requirements import volume_confirmation_required_features
...
class VolumeConfirmationRuleStrategy(Strategy):

    @property
    def metadata(self) -> StrategyMetadata:
        return StrategyMetadata(
            name="volume_confirmation_rule",
            version="0.1.0",
            category=StrategyCategory.VOLUME,
            status=StrategyStatus.EXPERIMENTAL,
            description="Rule-based volume confirmation strategy.",
            required_features=volume_confirmation_required_features(),
    ...
    def parameter_schema(self) -> StrategyParameterSchema:
        return StrategyParameterSchema(
            strategy_name="volume_confirmation_rule",
            parameters=[
                StrategyParameterSpec(name="min_score", param_type="float", default=50.0, min_value=0.0, max_value=100.0),
                StrategyParameterSpec(name="watch_only", param_type="bool", default=True),
                StrategyParameterSpec(name="min_relative_volume", param_type="float", default=1.2)
            ]
        )

    def _build_definition(self, min_score: float, min_relative_volume: float) -> RuleStrategyDefinition:
        return RuleStrategyDefinition(
            name="volume_confirmation_rule",
            family=RuleStrategyFamily.VOLUME_CONFIRMATION,
            description="High volume and positive volume trend confirmation",
            conditions=[
                RuleCondition(
                    name="high_relative_volume",
                    feature_name="volume_relative_volume_20",
                    operator=RuleConditionOperator.GT,
                    threshold=min_relative_volume,
                    weight=40.0,
                    description="Relative volume > threshold"
                ),
                RuleCondition(
                    name="positive_dollar_volume",
                    feature_name="dollar_volume",
                    operator=RuleConditionOperator.GT,
                    threshold=0.0,
                    weight=30.0,
                    description="Dollar volume is positive"
                ),
                RuleCondition(
                    name="positive_volume_trend",
                    feature_name="volume_trend_strength_basic_20",
                    operator=RuleConditionOperator.GT,
                    threshold=0.0,
                    weight=30.0,
                    description="Volume trend strength is positive"
                )
            ],
    ...
    def generate_signals(self, batch: StrategyInputBatch, params: Optional[Dict[str, Any]] = None) -> List[StrategySignal]:
        ...
        definition = self._build_definition(p["min_score"], p["min_relative_volume"])
        signals = []
