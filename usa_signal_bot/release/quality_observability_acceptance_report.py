...
def build_quality_observability_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []

    target_area = AcceptanceAreaKind.QUALITY
    ...
    for item in bundle.evidence_items:
        if item.area_kind == target_area:
            ...
            if not item.valid:
                passed = False
                findings.append(f"Invalid quality_observability evidence: {item.evidence_name}")

    for step in steps:
        if step.area_kind == target_area:
            if step.status != ReleaseCandidateStatus.PASSED:
                passed = False
                findings.append(f"Failed quality_observability step: {step.step_name}")
    ...
    report = AcceptanceAreaReport(
        ...
        title="Quality Observability Acceptance",
        status=status,
        passed=passed,
...
def validate_quality_observability_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def quality_observability_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Quality Observability Report: {report.title} [{report.status.value}]"]
