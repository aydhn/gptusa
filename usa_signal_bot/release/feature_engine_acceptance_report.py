...
def build_feature_engine_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []

    target_area = AcceptanceAreaKind.FEATURE_ENGINE
    ...
    for item in bundle.evidence_items:
        if item.area_kind == target_area:
            ...
            if not item.valid:
                passed = False
                findings.append(f"Invalid feature_engine evidence: {item.evidence_name}")

    for step in steps:
        if step.area_kind == target_area:
            if step.status != ReleaseCandidateStatus.PASSED:
                passed = False
                findings.append(f"Failed feature_engine step: {step.step_name}")
    ...
    report = AcceptanceAreaReport(
        ...
        title="Feature Engine Acceptance",
        status=status,
        passed=passed,
...
def validate_feature_engine_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def feature_engine_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Feature Engine Report: {report.title} [{report.status.value}]"]
