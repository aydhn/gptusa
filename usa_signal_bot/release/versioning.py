...
def get_git_commit_safe(project_root: Optional[Path] = None) -> Optional[str]:
    try:
        ...
        result = subprocess.run(["git", "rev-parse", "HEAD"], cwd=cwd, capture_output=True, text=True, timeout=2)
...
def get_git_branch_safe(project_root: Optional[Path] = None) -> Optional[str]:
    try:
        cwd = project_root if project_root else Path.cwd()
        result = subprocess.run(["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=cwd, capture_output=True, text=True, timeout=2)
        if result.returncode == 0:
            return result.stdout.strip()
    except Exception:
        pass
...
def build_release_version(version: Optional[str] = None, project_root: Optional[Path] = None) -> ReleaseVersion:
