...
def build_safety_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []
    ...
    for item in bundle.evidence_items:
        ...
        if not item.read_only or not item.local_only:
            passed = False
            findings.append(f"Safety violation in evidence: {item.evidence_name}")

    for step in steps:
        if step.executed_real_side_effect or step.used_broker or step.mutated_paper_state or step.deployed or step.used_network or step.sent_telegram:
            passed = False
            findings.append(f"Safety violation in step: {step.step_name}")
    ...
    report = AcceptanceAreaReport(
        ...
        area_kind=AcceptanceAreaKind.SAFETY,
        title="Full System Safety Acceptance",
        status=status,
        passed=passed,
    ...
    if not passed:
        report.risk_flags.append(AdvancedAcceptanceRiskFlag.SAFETY_ACCEPTANCE_FAILED)

    return report

def validate_safety_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def safety_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Safety Report: {report.title} [{report.status.value}]"]
