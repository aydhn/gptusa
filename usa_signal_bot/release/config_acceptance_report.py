...
def build_config_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []

    target_area = AcceptanceAreaKind.CONFIG
    ...
    for item in bundle.evidence_items:
        if item.area_kind == target_area:
            ...
            if not item.valid:
                passed = False
                findings.append(f"Invalid config evidence: {item.evidence_name}")

    for step in steps:
        if step.area_kind == target_area:
            if step.status != ReleaseCandidateStatus.PASSED:
                passed = False
                findings.append(f"Failed config step: {step.step_name}")
...
def validate_config_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def config_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Config Report: {report.title} [{report.status.value}]"]
