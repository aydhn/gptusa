...
def build_data_pipeline_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []

    target_area = AcceptanceAreaKind.DATA_PROVIDER
    ...
    for item in bundle.evidence_items:
        if item.area_kind == target_area:
            ...
            if not item.valid:
                passed = False
                findings.append(f"Invalid data_pipeline evidence: {item.evidence_name}")

    for step in steps:
        if step.area_kind == target_area:
            if step.status != ReleaseCandidateStatus.PASSED:
                passed = False
                findings.append(f"Failed data_pipeline step: {step.step_name}")
    ...
    report = AcceptanceAreaReport(
        ...
        title="Data Pipeline Acceptance",
        status=status,
        passed=passed,
...
def validate_data_pipeline_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def data_pipeline_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Data Pipeline Report: {report.title} [{report.status.value}]"]
