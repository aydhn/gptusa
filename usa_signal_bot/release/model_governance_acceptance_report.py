...
def build_model_governance_acceptance_report(bundle: AcceptanceEvidenceBundle, steps: List[AdvancedDryRunStep]) -> AcceptanceAreaReport:
    evidence_ids = []
    passed = True
    findings = []

    target_area = AcceptanceAreaKind.ML_GOVERNANCE
    ...
    for item in bundle.evidence_items:
        if item.area_kind == target_area:
            ...
            if not item.valid:
                passed = False
                findings.append(f"Invalid model_governance evidence: {item.evidence_name}")

    for step in steps:
        if step.area_kind == target_area:
            if step.status != ReleaseCandidateStatus.PASSED:
                passed = False
                findings.append(f"Failed model_governance step: {step.step_name}")
    ...
    report = AcceptanceAreaReport(
        ...
        title="Model Governance Acceptance",
        status=status,
        passed=passed,
...
def validate_model_governance_acceptance_report(report: AcceptanceAreaReport) -> List[str]:
    errors = []
...
def model_governance_acceptance_report_to_text(report: AcceptanceAreaReport, limit: int = 300) -> str:
    lines = [f"Model Governance Report: {report.title} [{report.status.value}]"]
