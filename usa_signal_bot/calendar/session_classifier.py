...
def classify_timestamp_session(
    timestamp_str: str, calendar: LocalMarketCalendar
) -> MarketSessionType:
    ...
    if (
        ...
        or " " not in timestamp_str
        and "T" not in timestamp_str
    ):
    ...
    time_part = timestamp_str[
        11:16
    ]  # extract HH:MM assuming YYYY-MM-DDTHH:MM... or YYYY-MM-DD HH:MM...
    if len(time_part) < 5:
...
def classify_bar_session(
    row: dict[str, Any], calendar: LocalMarketCalendar
) -> MarketSessionType:
    timestamp = row.get("timestamp") or row.get("date") or ""
    return classify_timestamp_session(str(timestamp), calendar)


def _get_session_key(session: Any) -> str:
    return session.value if hasattr(session, "value") else str(session)


def _is_early_close(
    session: MarketSessionType, timestamp: str, calendar: LocalMarketCalendar
) -> bool:
    if session != MarketSessionType.REGULAR:
        return False
    if not timestamp or len(timestamp) < 10:
        return False
    return calendar.is_early_close(timestamp[:10])


def classify_rows_by_session(
    ...
    for r in rows:
        ...
        if _is_early_close(session, timestamp, calendar):
            summary[MarketSessionType.EARLY_CLOSE.value] += 1
        else:
            summary[_get_session_key(session)] += 1
