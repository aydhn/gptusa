...
def attach_calendar_metadata_to_provider_response(response: ProviderResponse, calendar: LocalMarketCalendar) -> ProviderResponse:
    ...
    validation_results = validate_provider_response_calendar_alignment(response, calendar)
...
def provider_response_calendar_summary(response: ProviderResponse, calendar: LocalMarketCalendar) -> dict[str, Any]:
    validation_results = validate_provider_response_calendar_alignment(response, calendar)

    total_missing = sum(r.missing_trading_days for r in validation_results)
    total_non_trading = sum(r.non_trading_day_rows for r in validation_results)
    invalid_symbols = sum(1 for r in validation_results if r.status == SessionValidationStatus.INVALID)
    warning_symbols = sum(1 for r in validation_results if r.status == SessionValidationStatus.WARNING)
...
def calendar_quality_adjustment_from_session_validation(result: SessionValidationResult) -> dict[str, Any]:
    ...
    if result.non_trading_day_rows > 0:
        ...
        adjustment["reasons"].append(f"Contains {result.non_trading_day_rows} non-trading day rows.")

    if result.missing_trading_days > 5:
        adjustment["score_multiplier"] *= 0.85
        adjustment["reasons"].append(f"Missing {result.missing_trading_days} trading days.")

    return adjustment

def provider_quality_with_calendar_adjustment(score: ProviderQualityScore, session_results: list[SessionValidationResult]) -> ProviderQualityScore:
...
def validate_provider_response_calendar_alignment(response: ProviderResponse, calendar: LocalMarketCalendar) -> list[SessionValidationResult]:
