...
def count_trading_days(calendar: LocalMarketCalendar, start_date: str, end_date: str) -> int:
    return len(calendar.trading_days_between(start_date, end_date))

def align_rows_to_trading_days(rows: list[dict[str, Any]], calendar: LocalMarketCalendar) -> dict[str, Any]:
    if not rows:
...
def non_trading_day_rows(rows: list[dict[str, Any]], calendar: LocalMarketCalendar) -> list[dict[str, Any]]:
    non_trading = []
    for r in rows:
        d_val = r.get("date") or r.get("timestamp") or ""
        if isinstance(d_val, str) and len(d_val) >= 10:
            date_str = d_val[:10]
            if not calendar.is_trading_day(date_str):
                non_trading.append(r)
    return non_trading
