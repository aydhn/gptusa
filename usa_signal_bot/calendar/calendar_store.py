...
from usa_signal_bot.core.exceptions import DataStorageError
...
def write_market_sessions_jsonl(path: Path, sessions: list[MarketSession]) -> Path:
    ...
    try:
        ...
        raise DataStorageError(f"Failed to write sessions to {path}: {e}")


def write_trading_day_results_jsonl(
    path: Path, results: list[TradingDayResult]
) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
        with open(path, "w", encoding="utf-8") as f:
        ...
        raise DataStorageError(f"Failed to write trading day results to {path}: {e}")


def write_session_validation_result_json(
    path: Path, result: SessionValidationResult
) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    try:
        with open(path, "w", encoding="utf-8") as f:
        ...
        raise DataStorageError(f"Failed to write session validation to {path}: {e}")
...
def write_calendar_review_result_json(path: Path, result: CalendarReviewResult) -> Path:
    ...
    try:
        ...
        raise DataStorageError(f"Failed to write calendar review to {path}: {e}")


def read_calendar_review_result_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise DataStorageError(f"Calendar review file not found: {path}")
    try:
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        raise DataStorageError(f"Failed to read calendar review from {path}: {e}")
...
def calendar_store_summary(data_root: Path) -> dict[str, Any]:
    ...
    validations_count = (
        len(list(validations_dir.glob("*.json"))) if validations_dir.exists() else 0
    )
    sessions_dir = calendar_sessions_dir(data_root)
    sessions_count = (
        len(list(sessions_dir.glob("*.jsonl"))) if sessions_dir.exists() else 0
    )
