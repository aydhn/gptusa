...
from usa_signal_bot.scheduler.scheduler_models import (
    ...
    ConcurrencyPolicy,
    ConcurrencyDecisionResult,
    create_concurrency_decision_id,
)
...
class ConcurrencyGuard:
    def __init__(
        self,
        lock_manager: FileRunLockManager,
        policies: Optional[List[ConcurrencyPolicy]] = None,
    ):
    ...
    def evaluate(self, scope: RunLockScope) -> ConcurrencyDecisionResult:
        ...
        active_locks = [
            l for l in all_locks if not self.lock_manager.is_lock_stale(l, now_utc)
        ]
    ...
    def acquire_or_block(
        ...
        scope: RunLockScope,
        owner: RunIdentity,
        mode: Optional[LockAcquisitionMode] = None,
    ) -> LockAcquisitionResult:
        policy = policy_for_scope(scope, self.policies)
        res = self.evaluate(scope)

        if (
            res.decision == ConcurrencyDecision.BLOCK
            and mode != LockAcquisitionMode.DRY_RUN
        ):
    ...
    def guard_context(
        ...
        if not acq_result.acquired and acq_result.mode != LockAcquisitionMode.DRY_RUN:
            raise RuntimeError(
                f"Concurrency Blocked for scope {scope.value}: {acq_result.errors}"
            )
        ...
        try:
            ...
            if acq_result.lock:
                try:
                    ...
                    logging.warning(
                        f"Failed to release lock during concurrency guard cleanup: {e}"
                    )
