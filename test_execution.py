...
sys.modules['usa_signal_bot.core.enums'] = MagicMock(
    CleanupRunStatus=DummyEnum,
    CleanupCandidateStatus=DummyEnum,
    RetentionArtifactType=DummyEnum,
    RetentionPolicyAction=DummyEnum,
    DiskQuotaStatus=DummyEnum,
    CleanupSafetyStatus=DummyEnum,
    RetentionReportType=DummyEnum
)

CleanupCandidateStatus.CANDIDATE = DummyEnum('CANDIDATE')
CleanupCandidateStatus.REVIEW_REQUIRED = DummyEnum('REVIEW_REQUIRED')
CleanupCandidateStatus.DELETED = DummyEnum('DELETED')
CleanupCandidateStatus.FAILED = DummyEnum('FAILED')
CleanupCandidateStatus.PROTECTED = DummyEnum('PROTECTED')
CleanupCandidateStatus.SKIPPED = DummyEnum('SKIPPED')

CleanupRunStatus.COMPLETED = DummyEnum('COMPLETED')
CleanupRunStatus.PARTIAL_SUCCESS = DummyEnum('PARTIAL_SUCCESS')
CleanupRunStatus.FAILED = DummyEnum('FAILED')
CleanupRunStatus.EMPTY = DummyEnum('EMPTY')
CleanupRunStatus.DRY_RUN_COMPLETED = DummyEnum('DRY_RUN_COMPLETED')


RetentionArtifactType.UNKNOWN = DummyEnum('UNKNOWN')
RetentionPolicyAction.DELETE = DummyEnum('DELETE')
