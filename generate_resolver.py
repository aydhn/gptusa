...
from usa_signal_bot.integration.phase158_models import IntegrationInputReference, IntegrationInputKind
...
def build_integration_input_references(payloads: Dict[str, Any]) -> List[IntegrationInputReference]:
    references = []
    for key, value in payloads.items():
        ...
        if forbidden:
            ...
            ref.risk_flags.append(FullSystemIntegrationRiskFlag.FORBIDDEN_INTEGRATION_FIELD)
...
def validate_integration_input_references(items: List[IntegrationInputReference]) -> List[str]:
    violations = []
    for item in items:
        if not item.valid:
            violations.append(f"Invalid reference {item.source_artifact_name}: forbidden fields {item.forbidden_fields_detected}")
    return violations

def integration_input_resolver_summary(items: List[IntegrationInputReference]) -> Dict[str, Any]:
    return {
...
def integration_input_resolver_to_text(items: List[IntegrationInputReference], limit: int = 300) -> str:
    summary = integration_input_resolver_summary(items)
    text = f"Input Resolver: {summary}"
    return text[:limit] + "..." if len(text) > limit else text
"""

with open("usa_signal_bot/integration/integration_input_resolver.py", "w") as f:
    f.write(content)
