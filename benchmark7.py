...
def run_benchmark(count, optimized=False):
    ...
    if optimized:
        ...
        def execute_optimized(plan, force=False):
            ...
            # Optimization: avoid deep copy c = CleanupCandidate(**...) which is done in execute_candidate
            # Optimization: avoid import inside loop
            ...
            for c in plan.candidates:
                if c.status in (mock_enums.CleanupCandidateStatus.CANDIDATE, mock_enums.CleanupCandidateStatus.REVIEW_REQUIRED):
                    # In place modify candidate to save object creation
                    ...
                    p = Path(c.path)
                    try:
                        # manual inline of safe_delete_path
                        try:
